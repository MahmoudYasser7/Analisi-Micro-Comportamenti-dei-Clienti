...
(1, 'Variazione Mensile del Fatturato', 'Variazione mese su mese del fatturato (bookings + ancillaries).',
...
(2, 'Fatturato per Canale', 'Fatturato totale suddiviso per canale (bookings / ancillaries / totale).',
N'SELECT
...
(3, 'Ricavo Medio per Destinazione', 'Ricavo medio (bookings + ancillaries) per destinazione.',
N'SELECT
...
(5, 'Ricavo Totale per Paese', 'Ricavo per paese (bookings / ancillaries / totale).',
N'SELECT
    [country] AS [Paese],
...
(6, 'Percentuale Ancillary vs Ticket per Utente', 'Percentuale ancillary rispetto al ticket per ogni utente.',
N'SELECT
    [user id] AS [ID Cliente],
...
(11, 'Ricavo per Stagione e Destinazione', 'Ricavo totale (bookings + ancillaries) per destinazione e stagione.',
N'SELECT
    [destination] AS [Destinazione],
    CASE
...
(14, 'Top 7 Utenti per Valore (2024)', 'Top 7 utenti per valore economico totale nell''anno 2024 (bookings + ancillaries).',
N'SELECT TOP 7
    [user id] AS [ID Cliente],
...
(15, 'Fatturato e Prenotazioni 2023-2024', 'Fatturato ticket, ancillary e totale per anno 2023 & 2024.',
N'SELECT
    YEAR([booking date]) AS [Anno],
...
(19, 'Ricavo Totale per Canale 2023-2024', 'Ricavo totale per canale per gli anni 2023 & 2024.',
N'SELECT
    [channel] AS [Canale],
    YEAR([booking date]) AS [Anno],
    SUM([booking revenue] + ISNULL([ancillary revenue],0)) AS [Ricavo Totale]
